...
pub fn build(b: *std.Build) void {
    ...
    if (gnu_objcopy) |objcopy_path| {
        const bin_filename = b.fmt("{s}.bin", .{"test02"});
    ...
    if (gnu_objdump) |objdump_path| {
        const lst_filename = b.fmt("{s}.lst", .{"test02"});
    ...
    const run_step = b.step("run", "Run rvddt with the generated test02.bin");
    run_step.dependOn(&rvddt.step);

    const lst_step = b.step("lst", "Generate test02.lst from firmware ELF using GNU objdump");
